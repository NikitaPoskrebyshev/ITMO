...
def male_age(df: pd.DataFrame) -> float:
    ...
    :return: mean age
    """
    condition = (df['Survived'] == 1) & (df['Sex'] == 'male') & (df['Embarked'] == 'S') & (df['Fare'] > 30)
    return df.loc[condition]['Age'].mean()
...
def class_distribution(df: pd.DataFrame) -> pd.Series:
    ...
    """
    return (df['Pclass'].value_counts() / len(df)).sort_index()
...
def families_count(df: pd.DataFrame, k: int) -> int:
    ...
    srs = df['Name'].apply(get_surname).value_counts()
    return len(srs[srs > k])
...
def dead_lucky(df: pd.DataFrame) -> float:
    ...
    def is_lucky(ticket: str) -> bool:
        n: int = len(ticket)
        if not ticket.isnumeric() or n % 2:
            return False
        return sum(map(int, list(ticket)[:n//2])) == sum(map(int, list(ticket)[n//2:]))
