...
def get_region_ids_with_none_replaces_by_default(
    ...
    :return: region ids
    """
    return [r if (r := i["RegionID"]) is not None else DEFAULT_REGION_ID for i in records]
...
def get_region_id_if_not_none(
    ...
    :return: region ids
    """
    return [r for i in records if (r := i["RegionID"]) is not None]
...
def get_record_with_none_if_key_not_in_keys(
    ...
    """
    return {k: r[k] if k in keys else None for k in r.keys()}
...
def get_record_with_key_in_keys(
    ...
    """
    return {k: r[k] for k in r.keys() if k in keys}
...
def get_keys_if_key_in_keys(
    ...
    """
    return {k for k in r.keys() if k in keys}
